...
class ParkingServiceTest {
    ...
    void handleEntry_shouldCreateRecordWithDynamicPrice() {
        ...
        verify(vehicleRecordRepository).save(argThat(r ->
                r.getLicensePlate().equals("ZUL0001") &&
                        r.getSector().equals("A") &&
                        r.getPriceApplied() == 11.0 &&
                        r.getStatus() == RecordStatus.ENTERED
        ));
    }
    ...
    void handleEntry_shouldThrowWhenFull() {
        ...
        assertThrows(IllegalStateException.class, () ->
                parkingService.handleEntry("ZUL0001", LocalDateTime.now())
        );
    ...
    void handleParked_shouldOccupySpotAndUpdateRecord() {
        when(vehicleRecordRepository.findTopByLicensePlateAndStatusNotOrderByEntryTimeDesc(
                "ZUL0001", RecordStatus.EXITED))
        ...
        verify(vehicleRecordRepository).save(argThat(r ->
                r.getSpotId() == 1L &&
                        r.getStatus() == RecordStatus.PARKED
        ));
    }
    ...
    void handleParked_shouldThrowWhenSpotNotFound() {
        ...
        assertThrows(IllegalStateException.class, () ->
                parkingService.handleParked("ZUL0001", -99.0, -99.0)
        );
    }
    ...
    void handleExit_shouldFreeSpotAndSaveRevenue() {
        ...
        verify(revenueEntryRepository).save(argThat(r ->
                r.getSector().equals("A") &&
                        r.getAmount() == 20.0
        ));
        verify(vehicleRecordRepository).save(argThat(r ->
                r.getStatus() == RecordStatus.EXITED &&
                        r.getAmountCharged() == 20.0
        ));
    }
    ...
    void handleExit_shouldThrowWhenRecordNotFound() {
        when(vehicleRecordRepository.findTopByLicensePlateAndStatusNotOrderByEntryTimeDesc(
                "ZUL9999", RecordStatus.EXITED))
                .thenReturn(Optional.empty());

        assertThrows(IllegalStateException.class, () ->
                parkingService.handleExit("ZUL9999", LocalDateTime.now())
        );
    }
}